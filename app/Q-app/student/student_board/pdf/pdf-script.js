            ...
            .then((data) => {
              if (Object.keys(data).length != 0) {
                ...
                if (responseresult.result == "success") {
                  ...
                  Swal.fire({
                    html: "<strong>答えを提出しました。</strong><br>(" + answer + ")",
                    title: "情報",
                    icon: "info",
                    toast: true,
                    position: "top-end",
...
window.onload = async function () {
  ...
  if (class_Code == "" || class_Code == undefined) {
    prevent_Overlogin();
    Swal.fire({
      html: "<strong>クラス情報が読み込めませんでした。</strong>(Code:CSE-01)",
      title: "エラー",
      icon: "error",
      timer: 1500,
    }).then((result) => {
  ...
  await fetch(url, {
    ...
    .then((data) => {
      ...
      if (data.length != 0) {
        ...
        Swal.fire({
          html: "<strong>このクラスにはPDF資料がありません。</strong><br>通常の解答画面に戻ります。",
          icon: "info",
        }).then((result) => {
          if (result.isConfirmed) {
...
function showPDF() {
  ...
  if (pdfzone.src != "https://r2-pdf.cla-q.shakenokiri.me/" + pdffilename) {
    pdfzone.src = "https://r2-pdf.cla-q.shakenokiri.me/" + pdffilename;
  }
}
